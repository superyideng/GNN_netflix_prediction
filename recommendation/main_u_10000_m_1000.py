...
from rec.datasets.movielens_u_10000_m_1000 import MovieLens
#from rec.datasets.movielens_new import MovieLens
...
parser.add_argument('--n-negs', type=int, default=2)
...
if os.path.exists(cache_file):
    ...
    #ml = MovieLens('./ml-1m')
    ml = MovieLens('./ml-1m_demo')
    #ml = MovieLens('./ml-1m_mh')
...
def train():
    ...
    for epoch in range(100): # 500!!!
        ...
        print(len(g_train_bases))
        print(len(g_train_pairs))
         
        ######
        print('Epoch %d validation' % epoch)
        with torch.no_grad():
            print(len(g_test_bases))
