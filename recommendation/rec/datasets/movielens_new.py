...
class MovieLens(object):
    def __init__(self, directory):
        ...
        with open(os.path.join(directory, 'movies.dat'), encoding='latin1') as f:
            for l in f:
                ...
                try:
                    ...
                    continue
                    print(title)
                    data ={'id': int(id_),'title': title[:-6].strip(),'year':''}
                for g in genres_set:
                    data[g] = True
                movies.append(data)
        self.movies = (
        ...
        print('===========movies=============')
        print(self.movies.iloc[0:2])
        print('==============================')

		# read ratings
        #with open(os.path.join(directory, 'ratings_400.dat')) as f:
        with open(os.path.join(directory, 'ratings_400_shuf_10000.dat')) as f:
            for l in f:
        ...
        '''
        print("==========thisid===================")
        thisid = self.ratings['movie_id'].values[4388]
        print(thisid)
        '''
    ...
    def data_split(self):
        user_cnt = pd.DataFrame(self.ratings.groupby('user_id')['rating'].count()).reset_index()
        user_cnt.columns=['user_id','user_cnt']
        self.ratings = pd.merge(self.ratings, user_cnt, on=['user_id', 'user_id'])
        self.ratings = self.ratings[self.ratings.user_cnt>=2]
        self.ratings.drop(['user_cnt'], axis=1, inplace=True)
    ...
    def build_graph(self):
        ...
        print ("=========user_ids_invmap===len of users is============")
        #print(self.user_ids_invmap[1])
        print(len(user_ids_invmap))
        print("==========movie_ids_invmap====len of movies is===========")
        #print(self.movie_ids_invmap[1])
        print(len(movie_ids_invmap))
        ...
        '''
        print('========thisid-index map================')
        thisid = self.ratings['movie_id'].values[4388]
        print(movie_ids_invmap[thisid])
        print(len(user_ids))
        print('====================================')
        '''
